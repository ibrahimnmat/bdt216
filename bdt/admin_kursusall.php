...
?> <fieldset><legend><strong>Maklumat Pengkhususan Pelajar</strong></legend>
	<table align="center">
	<tr>
	<td>
...
<?php
	require_once('include/mysql_connect.php'); 
		$query 		= "SELECT COUNT(*) AS rowNo FROM kursus";
		$rowData 	= 0;
		...
		if($num>0){
			...
			echo "<a href=admin_kursusall.php?limit=all&page=1>Semua</a>";
			echo "&nbsp&nbsp&nbsp";
			echo "<a href=admin_kursusall.php?limit=10&page=1>10</a>";
			echo "&nbsp&nbsp&nbsp";
			echo "<a href=admin_kursusall.php?limit=20&page=1>20</a>";
			echo "&nbsp&nbsp&nbsp";
	?>
		</td>
		</tr></table>
 <br>
        <div align="center"> <a href="admin_kursustambah.php">
<u><strong><font color="#0000CC">+ Tambah Maklumat Pengkhususan Pelajar</font></strong></u></a></div>
...
<?php
	$query = "SELECT * FROM kursus ORDER BY kursusDesc";
		if($_REQUEST['limit']>0 && $_REQUEST['page']<2)
			...
			if ($result = $link->query($query)){
				...
				echo "</td>";
				echo "<td bgcolor=RED>"; ?>
 <a href="admin_kursusdelete.php?id=<?php echo $row['kursusDesc']; ?>" onclick="return confirm('Adakah Anda Pasti Untuk Padam Rekod ini?')"> <font color=yellow>PADAM</font> </a> <?php
				echo "</td>";
				echo "</tr>";
					$tmpID++;
					}
...
<?php
		...
		if($_REQUEST['page']>2)
			...
			$prev = 1;
			echo "<a href='admin_kursusall.php?limit=".$_REQUEST['limit'].
				 "&page=".$prev."'>Sebelum</a>";
			echo "&nbsp&nbsp&nbsp";	
			$int_i=1;	 													
