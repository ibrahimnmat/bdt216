...
echo "<p><font color='red' size='+1'>Anda TIDAK dibenarkan untuk melihat kandungan dalam halaman ini !!</font></p>";
	...
	if(isset($_POST['submitted'])){//Delete files
			...
			if($affect>0){
				...
				echo "<div align=\"center\"><a href=\"pensyarah_pelajar.php\"><input type=\"submit\" value=\"KEMBALI\"></div>";
			}else{
	...
	$query = "SELECT file_name, file_size, description, file_who, Date_FORMAT(date_entered, '%M %e, %Y') FROM uploads WHERE upload_id=$uid";
	$result = $link->query($query);
	
	list($fn,$fs,$d,$l,$date) = mysqli_fetch_array($result,MYSQL_NUM);
	?>
		<fieldset><legend><strong>PADAM RPH FAIL </strong></legend>
<form action="pensyarah_RPHdelete.php" method="post">
