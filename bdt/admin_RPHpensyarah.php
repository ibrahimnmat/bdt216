...
echo "<font color='red' size='+1'>Anda TIDAK dibenarkan untuk melihat kandungan dalam halaman ini !!</font>";
	...
	$first = TRUE;
	?>
    <fieldset><legend><strong>SENARAI RPH YANG DIHANTAR OLEH PENSYARAH</strong><i>&nbsp;(<?php echo $name;?>)</i></legend>
    <br />
    <?php
	$query = "SELECT upload_id, file_name, ROUND(file_size/1024) As fs, description, file_who, Date_FORMAT(date_entered, '%M %e, %Y') 		    As d FROM uploads where file_comment='$id' ORDER BY date_entered DESC";
